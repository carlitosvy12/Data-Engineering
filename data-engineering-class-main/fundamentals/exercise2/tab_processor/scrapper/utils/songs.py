...
def get_songs(output_directory: str, version: int = 0):
    ...
    catalog_path = Path(output_directory) / "catalog.json"

    try:
        catalog = files.load_from_json(catalog_path)
    except Exception as e:
        log.error(f"Error loading catalog from {catalog_path}: {e}")
        raise

    log.info("Starting lyrics download from catalog.json")

    
    for artist in catalog:
        artist_name = artist.get("name", "Unknown artist")
        log.info(f"Processing artist: {artist_name}")

        for song in artist.get("songs", []):
            song_title = song["song_title"]
            song_url = song["song_url"]
            song_file_path = song["lyrics_path"]   

            try:
                downloaded = get_song_lyrics(song_title, song_url, song_file_path)
                ...
                if downloaded:
                    ...
                    e
                    log.info(f"Skipping download for existing file: {song_file_path}")

            except Exception as e:
                log.error(
                    f"Error downloading '{song_title}' from {song_url}: {e}"
                )
                continue

    log.info("Lyrics download from catalog completed.")
